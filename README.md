...
- **`papermd/`** — a small crate that converts academic PDFs to Markdown.
  Has two backends behind feature flags:
  - `local` — subprocesses [MinerU](https://github.com/opendatalab/MinerU)
    via `uv` (requires Python + uv on the machine).
  - `remote` — HTTP client that talks to a MinerU FastAPI server (or any
    server speaking the same simple `POST /v1/convert` API).
...
Early scaffolding. See `eprint/CHANGELOG.md` (TBD) for progress.
